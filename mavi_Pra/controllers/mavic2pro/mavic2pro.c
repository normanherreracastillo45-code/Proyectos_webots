...
int main(int argc, char **argv) {
  ...
  while (wb_robot_step(timestep) != -1) {
   ...
   // --- BLOQUE DE DETECCIÓN DE OBJETOS (HUMANO Y ÁRBOL) ---
    ...
    if (number_of_objects > 0) {
      ...
      for (int i = 0; i < number_of_objects; ++i) {
        const char *model_name = objects[i].model;

        // 1. DETECCIÓN DE HUMANO
        if (strstr(model_name, "pedestrian") != NULL || strstr(model_name, "Pedestrian") != NULL || strstr(model_name, "man") != NULL || strstr(model_name, "woman") != NULL) {
          printf("\nObjeto humano detectado\n");
          printf("Coordenadas GPS -> X: %.2lf, Y: %.2lf, Z: %.2lf\n", gps_values[0], gps_values[1], gps_values[2]);
        }
        
        // 2. DETECCIÓN DE ÁRBOL
        else if (strstr(model_name, "tree") != NULL || strstr(model_name, "Tree") != NULL || strstr(model_name, "oak") != NULL || strstr(model_name, "pine") != NULL) {
          printf("\nObjeto árbol detectado\n");
          printf("Coordenadas GPS -> X: %.2lf, Y: %.2lf, Z: %.2lf\n", gps_values[0], gps_values[1], gps_values[2]);
        }
      }
    }

  }; // <-- Esta es la llave de cierre que ya tienes al final de tu imagen
